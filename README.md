...
- Now that the Graph has stored the data in its node's in the form of `entities` we defined, We can query the Graph node for this data using the [GraphQL Endpoint](https://graphql.org/learn/)
...
- Create an abi.json file inside your `RandomWinnerGame` folder(You will need to this file to intilialize your graph) and copy the following [content](https://github.com/LearnWeb3DAO/Graph/blob/master/abi.json). Note this is the abi of the `RandomWinnerGame` contract you created in the Chainlink VRF tutorial
...
- Next execute the following commmand:
  ...
  - For the deploy key, go to [The Graph's Hosted Service](https://thegraph.com/hosted-service/) and click on `My Dashboard` and copy the `Access Token` and paste it for the `Deploy Key`
...
You can go back to [The Graph's Hosted Service](https://thegraph.com/hosted-service/) and click on `My Dashboard` you will be able to see your graph

![](https://i.imgur.com/OOEbDgB.png)

Boom you have deployed your first graph!!!

Here comes the fun part, where we will modify the default code provided to us by The Graph into the code which can help us track events for our contract

**Lets get started**

- Open up you `subgraph.yaml` inside the `graph` folder and add the a `startBlock` to the yaml file after the `abi: RamdomWinnerGame` line, to get the startBlock you will need to go to [Mumbai PolygonScan](https://mumbai.polygonscan.com/) and search up your contract address, after that you will need to copy the block number of the block in which your contract was deployed
...
- Here`ID` is the unique indetifier for a game and will be equivalent to the `gameId` variable we had in our contract.
...
- `winner` is the address of the winner in the game and its defined as Bytes because address is a hexadecimal string
...
- After this `The Graph` will have created most of the code for you expect of the mappings.
...
- These functions get called everytime - Graph finds an event relating to these functions.
...
- Lets understand what is happening in `handleGameEnded` function
  ...
  - it takes in the `GameEnded` event and expects `void` which means nothing gets returned from the function
  ...
  - If it exists it set the winner and the requestId from the event into our Game Object(Note `GameEnded` event has the winner and requestId)
...
- Now you want the latest game every single time. To get the latest game, you will first have to order them by Id and then put this data in decending order so that gameId 3 can come at the top(It will be the current playing game) and then we say `first:1` because we only want the gameId 3 object, we dont care about the old games.
...
- You can see some data coming up for my graph 👀 and the id of the game is 3
